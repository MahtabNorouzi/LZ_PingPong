...
contract MyOAppTest is TestHelperOz5 {
    ...
    function setUp() public virtual override {
        ...
        aOApp = MyOApp(payable(_deployOApp(type(MyOApp).creationCode, abi.encode(address(endpoints[aEid]), address(this)))));

        bOApp = MyOApp(payable(_deployOApp(type(MyOApp).creationCode, abi.encode(address(endpoints[bEid]), address(this)))));
